...
- The provider's memory model is `UnifiedMemoryCap` (`provider-swift/Sources/ProviderCore/Inference/UnifiedMemoryCap.swift`): hard cap = 0.90 × physical RAM (always leaving ≥ 2 GiB for the OS; `DARKBLOOM_MEM_CAP_FRACTION` override). The model-load gate requires resident weights + incoming weights + headroom (the resolved activation reserve plus 1 GiB minimum KV) ≤ the cap, and a post-load guard unloads a freshly-loaded model whose measured live KV headroom is below the minimum serveable KV. The weights figure is the scanner's padded estimate (disk × 1.2) unless the catalog id has a **measured resident weights** entry (`measuredResidentWeightsBytes` — text-only artifacts only; a model whose artifact carries `vision_config` keeps the padding, because a text-path bench residency under-counts the tower `VLMModelFactory` loads). The pending-load reservation deliberately keeps the PADDED figure even for measured models — it guards the load transient, which the steady residency does not cover. The `DARKBLOOM_ACTIVATION_RESERVE_GB` env override is **raise-only against the resolved floor**; only programmatic `activationReserveBytes` values (tests) are honored as given. The coordinator's `freeMemoryAdmits`/`reportedFreeForLoadAdmits` mirrors this per (binary version, model): measured weights for ≥ `servabilityPerModelFloorMinVersion` binaries on measured models, padded otherwise.
